...
# MAGIC %md
# MAGIC ### 6. Ingest files from Sprints folder  
# MAGIC 1. Read the json file with dataframe reader API
# MAGIC 2. Add metadata columns
# MAGIC     - source file
# MAGIC     - Ingestion timestamp
# MAGIC 3. Write to bronze schmea as delta table
# MAGIC - Note: The file contains multiline data
# MAGIC
...
sprint_df = spark.read.format('json').\
                option('mode', 'FAILFAST').\
                schema(sprint_schema).\
                option('multiLine', True).\
                load("/Volumes/formula1/landing/files/sprints")
...
sprint_final_df = sprint_df.withColumn('ingestion_timestamp', F.current_timestamp()).\
...
display(spark.table("formula1.bronze.sprints"))
