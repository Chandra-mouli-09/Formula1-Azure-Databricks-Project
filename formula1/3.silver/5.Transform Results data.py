...
# MAGIC 1. Read Results data from bronze schema
...
results_df = spark.table("formula1.bronze.results")
...
results_selected_df = results_df.drop(col("url"))
...
results_distinct_df = results_filtered_df.dropDuplicates(["constructor_id", "driver_id", "round", "season"])
...
results_final_df.write.\
    format("delta").\
    mode("overwrite").\
    saveAsTable("formula1.silver.results")
